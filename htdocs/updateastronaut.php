<head>
    <link rel="stylesheet" href="styles.css?v=1" type="text/css">
</head>


<body>
    <div class="navbar">
        <div class="navbar__container">
            <ul class="navbar__menu">
            </br>
                <li class="navbar__btn">
                    <a href="/home.html" class="button">
                        Home
                    </a>
                </li>
            </ul>
        </div>
    </div>

    <?php
        include 'connect.php';
        $conn = OpenCon();
    ?>

    <div class="main">
        <div class="main__container-2">
            <form action="processupdateastronaut.php" method="post" class="main__content">
                <h1>Update astronaut attributes</h1>
                <h2>(Leave empty if no change)</h2>
                <h3>Select which astronaut to update:</h3>
                <?php
                    $result = $conn->query("select astroid,fname,lname from astronauts");
                    echo "<select name='astroid'>";
                    while($row = $result->fetch_assoc()) {
                        unset($astroid);
                        $astroid = $row['astroid'];
                        unset($name);
                        $fname = $row['fname'];
                        $lname = $row['lname'];
                        echo '<option value="'.$astroid.'">'.$fname." ".$lname.'</option>';
                    }
                    echo "</select>";
                ?>

                </br>
                </br>
                <input type="text" name="fname" placeholder="First Name">

                </br>
                </br>
                <input type="text" name="lname" placeholder="Last Name">

                </br>
                </br>
                <input type="text" name="dob" placeholder="Date of Birth YYYY-MM-DD">

                <input type="submit" value="Update" name="submit">
            </form>
        </div>
    </div>